import React, { useEffect, useState } from "react";
import type { FormEvent } from "react";
import Todos from "./components/Todos";
import Header from "./components/Header";

const api = "/api";

function App() {
	const [todos, settodos] = useState<
		{ todo: string; id: any; completed: Boolean }[]
	>([
		{
			todo: "",
			id: 0,
			completed: true,
		},
	]);
	const [popupActive, setPopupActive] = useState(false);
	let [isLoading, setIsLoading] = useState(false);

	useEffect(() => {
		setIsLoading(true);
		getApiData();
	}, []);

	const getApiData = async () => {
		setIsLoading(false);
		const response = await fetch(api);
		const data = await response.json();
		settodos([]);
		await data.map((item: any) => {
			const response = {
				todo: JSON.stringify(item.todo),
				id: item._id,
				completed: item.completed,
			};
			settodos((todos) => [...todos, response]);
		});
	};

	const setApiData = async (value: any) => {
		const data = await fetch(api, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
			body: JSON.stringify({ todo: value }),
		});
		await getApiData();
	};

	let inputSubmit = async (e: FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		let value = e.currentTarget.input.value;
		setIsLoading(true);
		await setApiData(value).then((e.currentTarget.input.value = null));
	};
	const arraylength = 0;

	return (
		<div className="App  bg-gray-900 h-screen w-full flex justify-center items-center ">
			<div className="container sm:w-5/12 sm:h-5/6 h-full w-full px-4 sm:px-10 bg-white rounded-lg overflow-auto">
				<div className="flex justify-between items-center  pt-5 ">
					<Header todos={todos} />
				</div>
				<div className=" flex justify-end sm:pr-5">
					<button
						onClick={() => {
							setPopupActive(true);
						}}
						className="flex justify-center items-center  bg-blue-900 border-2 text-white text-xl font-bold  rounded-full shadow h-16 w-16 "
					>
						+
					</button>
				</div>
				<div className="flex flex-col justify-center w-full  h-auto">
					{popupActive ? (
						<div className="  flex justify-center items-center ease-in-out">
							<div className="w-5/6 bg-white p-5 rounded-lg text-white">
								<div className="flex justify-between px-2 pb-1 text-gray-900">
									<h1 className="">Add a task</h1>
									<button
										onClick={() => setPopupActive(false)}
										className="flex justify-center items-center  bg-red-600 border-2 text-white rounded-full shadow h-6 w-6 "
									>
										x
									</button>
								</div>
								<form onSubmit={(e) => inputSubmit(e)}>
									<input
										required
										name="input"
										className="text-black w-full  shadow-[0_3px_10px_rgb(0,0,0,0.2)] outline-none p-2"
									/>
									<button className="exit-btn mt-3 sm:h-14 sm:w-32 w-20 h-12 flex justify-center items-center">
										CREATE TASK
									</button>
								</form>
							</div>
						</div>
					) : null}

					{isLoading ? (
						<div role="status" className="flex justify-center">
							<svg
								className="inline mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
								viewBox="0 0 100 101"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
									fill="currentColor"
								/>
								<path
									d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
									fill="currentFill"
								/>
							</svg>
						</div>
					) : (
						<Todos
							todos={todos}
							settodos={settodos}
							getdata={getApiData}
						/>
					)}
				</div>
			</div>
		</div>
	);
}

export default App;
